...
public final class KMeans<T extends Point> {
    ...
    public static final int NUMBER_OF_THREADS = Runtime.getRuntime().availableProcessors() + 1;
    ...
    public ArrayList<Cluster<T>> clusterize(int k, final SetOfPoints<T> data) throws IllegalArgumentException, NullPointerException {
        ...
        while (!stop) {
            for (int i = 0; i < NUMBER_OF_THREADS; i++) {
                tasks[i] = ex.submit(new KMeansIterationBlock(centroids, points, clusterization, i));
            }
            for (int i = 0; i < NUMBER_OF_THREADS; i++) {
                try {
                    tasks[i].get();
